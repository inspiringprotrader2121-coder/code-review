...
- Emit the JSON as soon as the evidence is sufficient. Keep each finding message
  to at most three concise sentences. Omit `originalCode`/`fixedCode` unless both
  are exact, minimal replacements of at most 20 lines each; never copy a whole
  function or file just to fill those fields.
