...
test('Google and GitHub OAuth sign-in link an existing workspace account and open its dashboard', async (t) => {
  ...
  const existingUser = db.createPasswordUser({
  ...
  const googleCallback = await app.request(
    `/auth/google/callback?code=google-code&state=${encodeURIComponent(googleState)}`,
    {
      headers: { cookie },
    },
  );
  ...
  const githubStart = await app.request('/auth/github?next=/connect', {
  ...
  globalThis.fetch = async (input) => {
    ...
    if (url === 'https://api.github.com/user')
      return Response.json({ id: 42, login: 'member-gh', name: 'Member' });
  ...
  const githubCallback = await app.request(
    `/auth/oauth/callback?code=github-code&state=${encodeURIComponent(githubState)}`,
    {
      headers: { cookie: githubCookie },
    },
  );
...
test('OAuth-only MFA reauthentication must return the already-linked provider identity', async (t) => {
  ...
  globalThis.fetch = async (input) => {
    ...
    if (url === 'https://api.github.com/user')
      return Response.json({ id: 99, login: 'other-user' });
    if (url === 'https://api.github.com/user/emails') return Response.json([]);
    throw new Error(`unexpected fetch ${url}`);
  };
  const callback = await app.request(
    `/auth/oauth/callback?code=other-code&state=${encodeURIComponent(state)}`,
    {
  ...
  assert.equal(
    new URL(callback.headers.get('location')!, 'https://example.test').pathname,
    '/auth/login',
  );
