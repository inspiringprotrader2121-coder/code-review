...
export function checkRateLimit(key: string, opts: RateLimitOptions, now = Date.now()): RateLimitResult {
  pruneExpired(now);
  const windowMs =
    Number.isFinite(opts.windowMs) && opts.windowMs > 0 ? Math.min(Math.floor(opts.windowMs), 7 * 24 * 3600_000) : 60_000;
  const max = Number.isFinite(opts.max) && opts.max >= 0 ? Math.min(Math.floor(opts.max), 1_000_000) : 1;
