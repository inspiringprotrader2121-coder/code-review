...
test(
  ...
  async (t) => {
    ...
    let resolveWorkerFinalized!: () => void;
    const workerFinalized = new Promise<void>((resolve) => {
      resolveWorkerFinalized = resolve;
    });
    const queue = new (class extends MemoryReviewQueue {
      override async releaseLockAndDrain(key: string) {
        const pending = await super.releaseLockAndDrain(key);
        resolveWorkerFinalized();
        return pending;
      }
    })();
