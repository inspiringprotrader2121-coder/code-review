...
test('five high-tier webhook reviews reserve visible runs and drain independently', { timeout: 5_000 }, async (t) => {
  ...
  const queuedResponses = await Promise.all(responses.map((response) => response.json() as Promise<{ reason?: string }>));
  assert.deepEqual(queuedResponses.map((response) => response.reason), Array(PRS.length).fill('enqueued'));
  ...
  assert.deepEqual(running.map((run) => run.pr).sort((a, b) => a - b), PRS);
  ...
  assert.deepEqual(
    db.listReviewRuns(tenant.id).map((run) => ({ pr: run.pr, status: run.status })).sort((a, b) => a.pr - b.pr),
...
class ReviewLifecycle {
  ...
  started(_job: { pr: number }): void {
    this.startedCount += 1;
    assert.ok(this.startedCount <= this.expected, 'worker started more reviews than the burst contains');
    if (this.startedCount === this.expected) this.resolveStarted();
  }

  finalized(): void {
    this.finalizedCount += 1;
    assert.ok(this.finalizedCount <= this.expected, 'worker finalized more reviews than the burst contains');
