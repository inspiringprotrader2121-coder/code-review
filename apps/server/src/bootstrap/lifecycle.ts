import { providerCapacityRegistryFor, type ReviewQueue } from '@orvex-review/queue';
...
import { startPeriodicRecovery } from '../application/worker/recovery-service.js';
...
import { processRoleRunsScheduler, processRoleRunsWorkers } from './topology.js';
...
export async function startApplicationLifecycle(
  ...
  const runsWorkers = processRoleRunsWorkers(config.topology.role);
  const runsScheduler = processRoleRunsScheduler(config.topology.role);
  const providerCapacityRegistry = providerCapacityRegistryFor(queue);
  let tempCleanupTimer: NodeJS.Timeout | undefined;
  let pruneTimer: NodeJS.Timeout | undefined;
  let stopWorker: (() => Promise<void>) | undefined;
  let stopNightly: (() => void) | undefined;
  let stopPeriodicRecovery: (() => void) | undefined;
  ...
  if (runsScheduler) {
    ...
    pruneOnce();
    pruneTimer = setInterval(pruneOnce, DAILY_MS);
    pruneTimer.unref();

    try {
      await providerCapacityRegistry?.initializeProviderCapacities();
    ...
    stopPeriodicRecovery = startPeriodicRecovery({ queue, config, alert, log });
  }

  const loadConfig = () => loadWorkerConfig(db, config);
  if (runsWorkers) {
    try {
      await providerCapacityRegistry?.assertProviderCapacitiesReady();
    } catch (err) {
      log.error('[server] provider fleet capacity readiness failed', err);
      void alert({
        event: 'provider-capacity-startup-failed',
        severity: 'critical',
        message: `Provider fleet capacity is not safe for workers: ${(err as Error).message}`,
      });
    ...
    stopWorker = dependencies.startWorker
      ? dependencies.startWorker(queue, config)
      : startWorkerLoop(queue, { db, loadConfig, config, enablePeriodicRecovery: false });
  }
  if (runsScheduler) {
    stopNightly = dependencies.startNightly
