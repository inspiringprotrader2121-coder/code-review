...
test(
  ...
  async (t) => {
    ...
    assert.deepEqual(
      ...
      { reviews: 8, luna: 8, minimax: 8, deepseek: 24, verify: 1, sandboxes: 8 },
    );
    ...
    assert.ok((providers.get('deepseek')?.peak ?? 0) <= 24);
    ...
    await assertCanSaturate(providerAdmission, 'deepseek', 24);
