...
rg -q -- '--update-pinned-image' "$SCRIPT"
rg -q -- 'requires --apply' "$SCRIPT"
rg -q -- 'must run as root' "$SCRIPT"
rg -q -- 'ORVEX_CODEX_EGRESS_BROKER_IMAGE=sha256' "$SCRIPT"
rg -q -- 'chattr -i' "$SCRIPT"
rg -F -q -- 'chattr +i' "$SCRIPT"
rg -q -- 'PIN_RELOCK_ENV=1' "$SCRIPT"
rg -q -- 'cleanup_private_state' "$SCRIPT"
rg -q -- 'previous configured broker image is unavailable and no matching running broker can be snapshotted' "$SCRIPT"
rg -q -- 'docker commit --pause=false' "$SCRIPT"
rg -q -- 'docker export' "$SCRIPT"
rg -q -- 'docker import' "$SCRIPT"
rg -q -- 'exported the running broker for rollback' "$SCRIPT"
rg -q -- 'local rollback snapshot of the running broker' "$SCRIPT"
rg -q -- 'restoring the previous pinned broker' "$SCRIPT"
...
rg -q -- 'EGRESS_MAX_CONCURRENT=10000' "$SCRIPT"
rg -q -- 'EGRESS_MAX_REQUESTS_PER_WINDOW=10000' "$SCRIPT"
rg -q -- '--ulimit nofile=65536:65536' "$SCRIPT"
