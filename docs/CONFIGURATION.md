...
| `ORVEX_PROCESS_ROLE` | enum; all, api, worker, or scheduler; default all | `all` | no / none | - | Process responsibility. all preserves the current single-process deployment; api serves HTTP only, worker runs reviews only, and scheduler runs maintenance/recovery only. |
...
| `ORVEX_MAX_CONCURRENT_REVIEWS` | integer; 1..10000; default 8 | `32` | no / none | - | Per-worker review slots. Production sets this high enough that host memory and disk refuse work first. Per-client concurrent/hourly caps still apply. |
| `ORVEX_CODEX_APIKEY_CONCURRENCY` | integer; 1..10000; default worker concurrency | `32` | no / none | - | API-key-authenticated Codex/Luna concurrency cap. It cannot raise the whole-review limit. |
| `ORVEX_PROVIDER_CONCURRENCY_<PROVIDER>` | integer-template; provider in LUNA, DEEPSEEK, MINIMAX; 1..10000; defaults to the applicable worker/Codex limit | `(inherited; not rendered)` | no / none | - | Bounded provider-specific concurrency family. Supported instances are LUNA, DEEPSEEK, and MINIMAX; arbitrary names are rejected by schema drift checks. |
| `ORVEX_PROVIDER_CONCURRENCY_LUNA` | integer; 1..32; default applicable worker/Codex limit | `32` | no / none | - | Luna provider-stage concurrency cap. |
| `ORVEX_PROVIDER_CONCURRENCY_DEEPSEEK` | integer; 1..64; default applicable worker/Codex limit | `64` | no / none | - | DeepSeek provider-stage concurrency cap. Independent diff chunks fan out within this bound; Redis fleet admission enforces the aggregate capacity. |
| `ORVEX_PROVIDER_CONCURRENCY_MINIMAX` | integer; 1..32; default applicable worker/Codex limit | `32` | no / none | - | MiniMax provider-stage concurrency cap. |
| `ORVEX_FLEET_CAPACITY_EPOCH` | string; 1..64 safe characters; default v1 | `review-scale-v4` | no / none | - | Version of the scheduler-owned Redis provider-capacity plan. Bump it only during a coordinated drained fleet capacity change. |
...
| `ORVEX_FLEET_PROVIDER_CONCURRENCY_LUNA` | integer; 1..10000; default local Luna provider limit | `10000` | no / none | - | Fleet-wide Luna provider-stage capacity registered by the scheduler. |
| `ORVEX_FLEET_PROVIDER_CONCURRENCY_DEEPSEEK` | integer; 1..10000; default local DeepSeek provider limit | `10000` | no / none | - | Fleet-wide DeepSeek provider-stage capacity registered by the scheduler. |
| `ORVEX_FLEET_PROVIDER_CONCURRENCY_MINIMAX` | integer; 1..10000; default local MiniMax provider limit | `10000` | no / none | - | Fleet-wide MiniMax provider-stage capacity registered by the scheduler. |
...
| `ORVEX_REVIEW_CONCURRENCY` | integer; 1..10000; default 8 | `32` | no / none | - | Maximum per-provider stages a single idle review can use after provider admission. Capacity is divided fairly among active reviews. |
...
| `ORVEX_MAX_SANDBOXES` | integer; positive; default 8; maximum 10000 | `10000` | no / none | - | Host-wide maximum concurrent internal sandbox containers. Must stay within host Docker/RAM budget. |
...
| `ORVEX_MAX_OTHER_CHARS` | integer; positive chars; default and maximum 2000 | `2000` | no / none | - | Other supporting context budget. |
