...
| `ORVEX_REVIEW_CONCURRENCY`                                                      | per-provider capacity available to one idle review; shared fairly among active reviews (default 8)                                                                                                                   |
| `ORVEX_PROVIDER_CONCURRENCY_LUNA` / `_DEEPSEEK` / `_MINIMAX`                    | provider-specific **per-worker** ceilings. The production profile uses Luna 8, DeepSeek 8, and MiniMax 8. Idle capacity can fan out large-PR shards; Redis-wide active-review count divides it fairly during bursts. |
...
| `ORVEX_MAX_SANDBOXES`                                                           | host-wide concurrent sandbox cap (default 8; production profile 32; code max 100)                                                                                                                                     |
