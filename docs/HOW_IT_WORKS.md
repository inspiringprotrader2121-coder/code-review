...
| `ORVEX_PROVIDER_CONCURRENCY_LUNA` / `_DEEPSEEK` / `_MINIMAX`                    | provider-specific **per-worker** ceilings. The production profile uses Luna 8, DeepSeek 8, and MiniMax 8. A review's two Flash discovery stages run in sequence, and verification shares the same Flash cap. |
