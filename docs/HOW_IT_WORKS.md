...
| `ORVEX_PROVIDER_CONCURRENCY_LUNA` / `_DEEPSEEK` / `_MINIMAX`                    | provider-specific local and Redis-coordinated call ceilings. The production profile uses Luna 8, DeepSeek 24, and MiniMax 8: eight high-tier reviews can run two Flash discovery stages plus one Flash verifier each. |
