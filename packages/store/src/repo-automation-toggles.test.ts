...
test('isRepoActionEnabled: opened/reopened follow reviewOnOpen, synchronize follows reviewOnPush, independently', () => {
  ...
  assert.equal(
    db.isRepoActionEnabled(100, 'acme/api', 'reopened'),
    true,
    'reopened follows the open toggle',
  );
  assert.equal(
    db.isRepoActionEnabled(100, 'acme/api', 'synchronize'),
    false,
    'push toggle correctly off',
  );
  ...
  assert.equal(
    db.isRepoActionEnabled(100, 'acme/api', 'synchronize'),
    true,
    'push toggle unaffected by the open toggle',
  );
});
...
test('the overall repo `enabled` toggle is independent of the two trigger toggles', () => {
  ...
  assert.equal(
    after.reviewOnOpen,
    true,
    'disabling the repo does not change the per-trigger settings',
  );
