...
export async function verifyFindings(
  ...
  const strictInstructions = [
    ...
    '- It claims behavior is "silent", "missing", "unvalidated", "unhandled", or "swallowed" when',
    '  the code shown EXPLICITLY does that thing — returns a labelled skip/error reason, logs or',
    '  reports the condition, or takes a deliberate guarded fallback (quote the line that does it).',
    '  Behavior the code implements explicitly and on purpose is not a silent omission. (Two of the',
    '  last benchmark\'s false positives were exactly this: an "silently omitted" claim against a',
    '  function returning an explicit skipped-reason, and a flagged HTTP fallback that was the',
