...
test('parseOrvexCommand', async (t) => {
  ...
  await t.test('parses core commands, case-insensitively', () => {
    ...
    assert.deepEqual(parseOrvexCommand('@orvex reviews remaining', '@orvex'), {
      kind: 'rate_limit',
    });
    assert.deepEqual(parseOrvexCommand('@orvex check rate limit', '@orvex'), {
      kind: 'rate_limit',
    });
    assert.deepEqual(parseOrvexCommand('@orvex how many reviews remaining?', '@orvex'), {
      kind: 'rate_limit',
    });
    assert.deepEqual(parseOrvexCommand('@orvex what is my usage', '@orvex'), {
      kind: 'rate_limit',
    });
    ...
    assert.deepEqual(parseOrvexCommand('@orvex how many reviews left', '@orvex'), {
      kind: 'rate_limit',
    });
  });

  await t.test(
    'rate-limit near-miss does not hijack real commands that mention usage/quota',
    () => {
      ...
      assert.equal(
        parseOrvexCommand('@orvex explain the rate limit middleware', '@orvex')?.kind,
        'prompt',
      );
      assert.equal(
        parseOrvexCommand('@orvex deep review of our quota module', '@orvex')?.kind,
        'prompt',
      );
      assert.equal(parseOrvexCommand('@orvex update the quota module', '@orvex')?.kind, 'prompt');
      assert.equal(
        parseOrvexCommand('@orvex investigate our rate limit middleware', '@orvex')?.kind,
        'prompt',
      );
      assert.equal(
        parseOrvexCommand('@orvex check the usage of this cache', '@orvex')?.kind,
        'prompt',
      );
      assert.equal(
        parseOrvexCommand('@orvex how to add rate limits to this endpoint', '@orvex')?.kind,
        'prompt',
      );
      assert.equal(
        parseOrvexCommand('@orvex check if the rate limit middleware handles bursts', '@orvex')
          ?.kind,
        'prompt',
      );
      assert.equal(
        parseOrvexCommand('@orvex how should we implement quota enforcement', '@orvex')?.kind,
        'prompt',
      );
  ...
  await t.test('parses resolve-conflicts variants', () => {
    assert.deepEqual(parseOrvexCommand('@orvex resolve conflicts', '@orvex'), {
      kind: 'resolve_conflicts',
    });
    assert.deepEqual(parseOrvexCommand('@orvex fix merge conflicts', '@orvex'), {
      kind: 'resolve_conflicts',
    });
  });
