...
describe('doc-audit', () => {
  it('detects testPathPatterns in table row', () => {
    ...
    assert.ok(
      getAuditDocIssues(md, 'docs/audit/slice-01.md').includes('test_path_patterns_in_table'),
    );
  });
  ...
  it('emits findings with rule ids', () => {
    const findings = auditFindingsFromContent('| x | testPathPatterns=foo |', 'audit.md');
